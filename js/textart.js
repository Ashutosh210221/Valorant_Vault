...
(function () {
  ...
  function renderGrid() {
    ...
    grid.innerHTML = items.map(item => {
      ...
      return `
      <article class="ta-card${isBig ? ' ta-card--big' : ''}" data-id="${item.id}" data-art="${encodeURIComponent(item.art)}" tabindex="0" role="button" aria-label="Copy ${escapeHtml(item.title)}">
  ...
  async function copyCard(card) {
    ...
    card._t = setTimeout(() => card.classList.remove('copied'), 1400);
  }
